...
def triple_gamma(tri_mat, dimension_index):
    """
    dimension_index: 0、1、2 表示求GIP的对象
    gamma = node_num / 每个切面的二范数之和
    """
    x_len = tri_mat.shape[dimension_index]
    norms_squared = []
    for i in range(x_len):
        if dimension_index == 0:
            norms_squared.append(np.linalg.norm(tri_mat[i, :, :]) ** 2)
        elif dimension_index == 1:
            norms_squared.append(np.linalg.norm(tri_mat[:, i, :]) ** 2)
        elif dimension_index == 2:
            norms_squared.append(np.linalg.norm(tri_mat[:, :, i]) ** 2)
    gamma = sum(norms_squared)
    return x_len / gamma if gamma != 0 else 0


def triple_gip(triple_adj, dimension_index):
    gamma = triple_gamma(triple_adj, dimension_index)
    x_len = triple_adj.shape[dimension_index]
    x_mat = np.eye(x_len)
    for i in range(x_len):
        for j in range(i + 1, x_len):
            if dimension_index == 0:
                mat_a, mat_b = triple_adj[i, :, :], triple_adj[j, :, :]
            elif dimension_index == 1:
                mat_a, mat_b = triple_adj[:, i, :], triple_adj[:, j, :]
            elif dimension_index == 2:
                mat_a, mat_b = triple_adj[:, :, i], triple_adj[:, :, j]
            similarity = math.exp(-gamma * np.linalg.norm(mat_a - mat_b) ** 2)
            x_mat[i, j] = similarity
            x_mat[j, i] = similarity
    return x_mat


def fuse_sim(sim1, sim2, mode):
    # sim2: GIP sim
    if mode == "GIP":
        return sim2
    if mode == "ORIGIN":
        return sim1
    node_num = sim1.shape[0]
