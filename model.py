...
class GNNModel(torch.nn.Module):
    def __init__(self, drug_number, disease_number, target_number, hidden_dim, conv_type):
        ...
        if conv_type == "GAT":
            self.drug_proj = Linear(disease_number * target_number, hidden_dim, weight_initializer='glorot', bias=True)
            self.drug_conv1 = GATConv(in_channels=hidden_dim, out_channels=hidden_dim, heads=2, concat=False)
            self.drug_conv2 = GATConv(in_channels=hidden_dim, out_channels=hidden_dim, heads=2, concat=False)

            self.disease_proj = Linear(drug_number * target_number, hidden_dim, weight_initializer='glorot', bias=True)
            self.disease_conv1 = GATConv(in_channels=hidden_dim, out_channels=hidden_dim, heads=2, concat=False)
            self.disease_conv2 = GATConv(in_channels=hidden_dim, out_channels=hidden_dim, heads=2, concat=False)

            self.target_proj = Linear(disease_number * drug_number, hidden_dim, weight_initializer='glorot', bias=True)
            self.target_conv1 = GATConv(in_channels=hidden_dim, out_channels=hidden_dim, heads=2, concat=False)
            self.target_conv2 = GATConv(in_channels=hidden_dim, out_channels=hidden_dim, heads=2, concat=False)
