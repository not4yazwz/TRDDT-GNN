...
y_ROC_list = []
y_PR_list = []
for train_index, val_index in kf.split(samples[:, :3], samples[:, 3]):
    fold += 1
    ...
    # # 计算GIP核矩阵
    # drug_gip_sim = triple_gip(new_adj_matrix, 0)
    # disease_gip_sim = triple_gip(new_adj_matrix, 1)
    # target_gip_sim = triple_gip(new_adj_matrix, 2)
    #
    # # 融合GIP 和 原相似矩阵
    # mode = "FUSE"
    # drug_sim = fuse_sim(drug_similarity_matrix, drug_gip_sim, mode)
    # disease_sim = fuse_sim(disease_similarity_matrix, disease_gip_sim, mode)
    # target_sim = fuse_sim(target_similarity_matrix, target_gip_sim, mode)
    drug_sim = drug_similarity_matrix
    disease_sim = disease_similarity_matrix
    target_sim = target_similarity_matrix

    # K近邻形成图
    drug_graph = k_matrix(drug_sim, 30)
    disease_graph = k_matrix(disease_sim, 30)
    target_graph = k_matrix(target_sim, 30)
...
print(
    'auc:{:.4f},aupr:{:.4f},f1_score:{:.4f},accuracy:{:.4f},recall:{:.4f},specificity:{:.4f},precision:{:.4f}'.format(
        ...
        metrics[5],
        metrics[6]))
# roc_df = pd.DataFrame({'ROC_X': x_ROC_list, 'ROC_Y': y_ROC_list})
# pr_df = pd.DataFrame({'PR_X': x_PR_list, 'PR_Y': y_PR_list})
